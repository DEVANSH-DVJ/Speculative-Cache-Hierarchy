...
L0D_REPLACEMENT=${11}   # prefetcher/*.l1d_repl
L1D_REPLACEMENT=${12}   # prefetcher/*.l1d_repl
L2C_REPLACEMENT=${13}   # prefetcher/*.l2c_repl
LLC_REPLACEMENT=${14}   # prefetcher/*.llc_repl
ITLB_REPLACEMENT=${15}  # prefetcher/*.itlb_repl
DTLB_REPLACEMENT=${16}  # prefetcher/*.dtlb_repl
STLB_REPLACEMENT=${17}  # prefetcher/*.stlb_repl

NUM_CORE=${18}         # tested up to 8-core system
...
if [ ! -f ./replacement/${L0D_REPLACEMENT}.l0d_repl ]; then
    echo "[ERROR] Cannot find L0D replacement policy"
        echo "[ERROR] Possible L0D replacement policy from replacement/*.l0d_repl"
    find replacement -name "*.l0d_repl"
    exit 1
fi
...
cp replacement/${L0D_REPLACEMENT}.l0d_repl replacement/l0d_replacement.cc
...
echo "L0D Replacement: ${L0D_REPLACEMENT}"
...
echo "Cores: ${NUM_CORE}"
BINARY_NAME="${BRANCH}-${L1I_PREFETCHER}-${L1D_PREFETCHER}-${L2C_PREFETCHER}-${LLC_PREFETCHER}-${ITLB_PREFETCHER}-${DTLB_PREFETCHER}-${STLB_PREFETCHER}-${BTB_REPLACEMENT}-${L1I_REPLACEMENT}-${L0D_REPLACEMENT}-${L1D_REPLACEMENT}-${L2C_REPLACEMENT}-${LLC_REPLACEMENT}-${ITLB_REPLACEMENT}-${DTLB_REPLACEMENT}-${STLB_REPLACEMENT}-${NUM_CORE}core-${19}"
...
cp replacement/lru.l0d_repl replacement/l0d_replacement.cc

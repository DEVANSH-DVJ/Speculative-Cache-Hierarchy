...
public:
    ...
    // constructor
    CACHE(string v1, uint32_t v2, int v3, uint32_t v2a, int v3a, uint32_t v4, uint32_t v5, uint32_t v6, uint32_t v7, uint32_t v8)
        : NAME(v1), NUM_SET(v2), NUM_WAY(v3), NUM_LINE(v4), NUM_SET_SPEC(v2a), NUM_WAY_SPEC(v3a), WQ_SIZE(v5), RQ_SIZE(v6), PQ_SIZE(v7), MSHR_SIZE(v8)
    {
    ...
    void add_mshr(PACKET *packet),
        ...
        fill_cache(uint32_t set, uint32_t way, PACKET *packet, uint8_t cache_nature),
    ...
    uint32_t get_set(uint64_t address, uint8_t cache_nature),
        ...
        timestamp_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type);
};

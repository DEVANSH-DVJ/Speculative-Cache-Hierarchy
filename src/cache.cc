...
void CACHE::handle_read() {
  ...
  for (uint32_t i = 0; i < MAX_READ; i++) {
    ...
    if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) &&
      ...
      if (way >= 0) { // read hit in normal hierarchy
      ...
      } else { // read miss in normal hierarchy
        ...
        if ((NUM_SET_SPEC != 0) &&
            (way >= 0)) { // read hit in speculative hierarchy
...
void CACHE::fill_cache(uint32_t set, uint32_t way, PACKET *packet,
  ...
  if (cache_nature == NORMAL_HIERARCHY) {
    ...
    DP(if (warmup_complete[packet->cpu]) {
      cout << "[" << NAME << "] " << __func__ << " set: " << set
           << " way: " << way;
      cout << " lru: " << spec_block[set][way].lru << " tag: " << hex
           << spec_block[set][way].tag
           << " full_addr: " << spec_block[set][way].full_addr;
...
int CACHE::invalidate_entry(uint64_t inval_addr, uint8_t cache_nature) {
  ...
  if (cache_nature == NORMAL_HIERARCHY) {
    ...
    // invalidate
    for (uint32_t way = 0; way < NUM_WAY_SPEC; way++) {
      if (spec_block[set][way].valid &&
          (spec_block[set][way].tag == inval_addr)) {
        ...
        DP(if (warmup_complete[cpu]) {
          cout << "[" << NAME << "] " << __func__ << " inval_addr: " << hex
               << inval_addr;
          cout << " tag: " << spec_block[set][way].tag
               << " data: " << spec_block[set][way].data << dec;
          cout << " set: " << set << " way: " << way
               << " lru: " << spec_block[set][way].lru
               << " cycle: " << current_core_cycle[cpu] << endl;
  ...
  while (commit_counter < COMMIT_WIDTH) {
    ...
    if ((way >= 0) && (NUM_SET_SPEC != 0)) {
      ...
      if (spec_packet->type == COMMIT_LOAD) {
        ...
        if (cache_type == IS_LLC) {
          target_way =
              llc_find_victim(spec_packet->cpu, spec_packet->instr_id,
                              target_set, block[target_set], spec_packet->ip,
                              spec_packet->full_addr, spec_packet->type);
        } else {
          target_way =
              find_victim(spec_packet->cpu, spec_packet->instr_id, target_set,

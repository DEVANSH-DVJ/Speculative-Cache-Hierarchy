    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu])
    {
        ...
        if (block[set][way].dirty)
        {
            ...
            if (lower_level)
            {
                ...
                {
                    //WQ_debug
                    // if(MSHR.entry[mshr_index].instr_id == 564798)
                    // {
                    //     cout<<"WQ is full instr_id 564798 mshr_index: " << mshr_index << " cache level: " << NAME << endl;
                    // }
                ...
                else
                {
                    ...
                    //@Sumon: WQ bug resolution.
                    // While adding to WQ of L1D, the translated variable is checked if it is completed or not.
                    // But while filling a packet into cache the translated information is lost, so we need to set it explicitly
                    writeback_packet.translated = COMPLETED;
...
void CACHE::handle_writeback()
{
    ...
    if (cache_type == IS_L1D || cache_type == IS_L1DT) // Get completed index in WQ, as it is non-fifo
    {
        ...
        {
            //@Sumon: another solution to the translation problem, where the translated bit is not set for L1D writeback packets
            // if(cache_type == IS_L1D)
                // cout<<"WQ L1D wq_index: "<<wq_index<<" translated: "<<WQ.entry[wq_index].translated<< " event_cycle: "<< WQ.entry[wq_index].event_cycle << " current_core_cycle: "<< current_core_cycle[cpu]<<endl;
            // if ((WQ.entry[wq_index].translated == COMPLETED || cache_type == IS_L1D) && (WQ.entry[wq_index].event_cycle <= current_core_cycle[cpu]))
    ...
    if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0))
    {
        ...
        { // writeback hit (or RFO hit for L1DT)
            ...
            //WQ_debug
            // if (WQ.entry[index].instr_id == 564798) {  
            ...
            // check fill level
            //@Sumon: Most probably dead code, as WRITEBACK packet is only added to WQ of lower level and not in the MSHR of the current level, so no need to retrurn data
        ...
        { // writeback miss (or RFO miss for L1DT)
            ...
            { // RFO miss
                ...
                { // this is a new miss
                    ...
                    //@Sumon: Send virtual address to lower level as L1D is VIPT
                    // As this is added to RQ of L1D, it takes care of passing PA to subsequent lower levels
                    new_packet.address = WQ.entry[index].full_virtual_address >> LOG2_BLOCK_SIZE;
                    new_packet.full_addr = WQ.entry[index].full_virtual_address;
                    new_packet.fill_l1d = 1;
                ...
                if (block[set][way].dirty)
                {
                    ...
                    if (lower_level)
                    {
                        ...
                        else
                        {
                            ...
                            //@Sumon doubt: make sure this address is virtual in case of L1D, I think this should be physical address for L1D, need to store it in block. I guess this can be handled in fill cache
                            //Most probably this is not an issue, take care later
                            writeback_packet.address = block[set][way].address;
                            writeback_packet.full_addr = block[set][way].full_addr;
                            writeback_packet.data = block[set][way].data;
    ...
    for (uint32_t i = 0; i < MAX_READ; i++)
    {
        ...
        if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0))
        {
            ...
            { // read hit
                ...
                //WQ_debug
                // if(RQ.entry[index].instr_id == 564798) {
                // cout << "[" << NAME << "] " << __func__ << " read hit";
                // cout << " instr_id: " << RQ.entry[index].instr_id << " address: " << hex << RQ.entry[index].address;
                // cout << " full_addr: " << RQ.entry[index].full_addr;
                // cout << " virtual_addr: " << RQ.entry[index].full_virtual_address;
                ...
                if (RQ.entry[index].fill_level < fill_level)
                {
                    ...
                    {
                        //@Sumon: returns translation packet to PTW
                        extra_interface->return_data(&RQ.entry[index]);
                    }
                    //@Sumon-Doubt: why data of RQ entry is not set for L2C?
                    // Answer: data usually contains physical address in case of TLBs
                    else if (cache_type == IS_L2C)
                    {
                ...
                { // this is a new miss
                    ...
                    else
                    {
                        ...
                        if (cache_type == IS_L1I || cache_type == IS_L1D || cache_type == IS_L1DT) //@Vishal: VIPT
                        {
                            ...
                            //@Sumon: send physical address incase of L1I, as lower level are PIPT
                            //@Sumon: need to send physical address from L1D to lower level as L1D is VIPT and lower levels are PIPT
                            else
                            {
    ...
    for (uint32_t i = 0; i < MAX_READ; i++)
    {
        ...
        if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0))
        {
            ...
            if ((cache_type == IS_L1D || cache_type == IS_L1I) && (PQ.entry[PQ.head].full_physical_address >> LOG2_PAGE_SIZE) == (UINT64_MAX >> LOG2_PAGE_SIZE))
            {
                ...
                DP(if (warmup_complete[PQ.entry[index].cpu]) { cout << "[" << NAME << "_PQ] " << __func__ << " prefetch_id: " << PQ.entry[index].prefetch_id << "from handle prefetch on dropping prefetch request"
            ...
            { // prefetch hit
                ...
                if (PQ.entry[index].pf_origin_level < fill_level)
                {
                    ...
                    else if (cache_type == IS_ITLB)
                    {
                        ...
                        DP(if (warmup_complete[PQ.entry[index].cpu]) { cout << "[" << NAME << "_PQ] " << __func__ << " prefetch_id: " << PQ.entry[index].prefetch_id << "from handle prefetch on prefetch hit"
                                                                            << "instruction : " << PQ.entry[index].instruction << endl; });
                    }
                    else if (cache_type == IS_DTLB)
                    {
            ...
            { // prefetch miss
                ...
                // AGUS
                if (cache_type == IS_L2C && PQ.entry[index].type == PREFETCH)
                {
                    pf_miss_l1++;
                }
                // AGUS
                            ...
                            else
                            {
                                ...
                                if (PQ.entry[index].pf_origin_level < fill_level)
                                {
                                    ...
                                    else if (cache_type == IS_ITLB)
                                    {
                                        ...
                                        DP(if (warmup_complete[PQ.entry[index].cpu]) { cout << "[" << NAME << "_PQ] " << __func__ << " prefetch_id: " << PQ.entry[index].prefetch_id << "from handle prefetch"
                                                                                            << "instruction: " << PQ.entry[index].instruction << endl; });
                                    }
                                    else if (cache_type == IS_DTLB)
                                    {
    ...
    for (uint32_t way = 0; way < NUM_WAY; way++)
    {
        if (block[set][way].valid && (block[set][way].tag == packet_tag))
        {
            ...
            DP(if (warmup_complete[packet->cpu]) {
                            ...
                            << " virtual_addr: " << packet->full_virtual_address 
                            << " physical_addr: " << packet->full_virtual_address 
                            << " tag: " << block[set][way].tag << " data: " << block[set][way].data << dec;
...
int CACHE::add_rq(PACKET *packet)
{
    ...
    if (wq_index != -1)
    {
        ...
        if ((cache_type == IS_L1DT) && (packet->type != PREFETCH))
        {
            ...
            DP(if (warmup_complete[packet->cpu]) {
                            ...
                            cout << hex << " read: " << packet->address << " writeback: " << WQ.entry[wq_index].address;
                            cout << " virtual_addr: " << WQ.entry[wq_index].full_virtual_address;
                            cout << " physical_addr: " << WQ.entry[wq_index].full_physical_address << dec;
    ...
    if (index != -1)
    {
        ...
        if (cache_type == IS_L2C)
        {
            //@Sumon: Doubt, should I also add OR condition, fill_level == 0, I am not sure where this is used
            // resolved: packet already present in the read queue is data and the new packet is instruction with the same address, and vice-versa
            if (RQ.entry[index].fill_level == 1 && packet->fill_level == 1)
            {
...
void CACHE::update_fill_cycle()
{
    ...
    for (uint32_t i = 0; i < MSHR.SIZE; i++)
    {
        if ((MSHR.entry[i].returned == COMPLETED) && (MSHR.entry[i].event_cycle < min_cycle))
        {
            ...
            min_index = i;
                    ...
                    //WQ_debug
                    // if (MSHR.entry[i].instr_id == 564798) {
                    //     cout << "[" << NAME << "_MSHR] " <<  __func__ << " completed instr_id: " << MSHR.entry[i].instr_id;
                    //     cout << " address: " << hex << MSHR.entry[i].address << " full_addr: " << MSHR.entry[i].full_addr;
                    //     cout << " virtual_addr: " << MSHR.entry[i].full_virtual_address;
                    //     cout << " physical_addr: " << MSHR.entry[i].full_physical_address;
    ...
    if (min_index < MSHR.SIZE)
    {
        ...
        DP(if (warmup_complete[MSHR.entry[min_index].cpu]) {
                        ...
                        cout << " virtual_addr: " << MSHR.entry[min_index].full_virtual_address;
                        cout << " physical_addr: " << MSHR.entry[min_index].full_physical_address;
...
int CACHE::check_nonfifo_queue(PACKET_QUEUE *queue, PACKET *packet, bool packet_direction)
{
        ...
        for (uint32_t index = 0; index < queue->SIZE; index++)
        {
            if (queue->entry[index].address == check_address)
            {
                ...
                DP(if (warmup_complete[packet->cpu]) {
                                cout << "[" << NAME << "_" << queue->NAME << "] " << __func__ << " same entry instr_id: " << packet->instr_id << " prior_id: " << queue->entry[index].instr_id;
                                cout << " address: " << hex << packet->address;
                                cout << " full_addr: " << packet->full_addr << " check_addr: "<< check_address << dec << endl; });
    ...
    // WQ_debug
    // if (packet->instr_id == 564798 && (queue->occupancy == queue->SIZE)) { 
    //                 cout << "[" << NAME << "_" << queue->NAME << "] " << __func__ << " mshr is full";
    //                 cout << " instr_id: " << packet->instr_id << " occupancy: " << queue->occupancy;
    //                 cout << " address: " << hex << packet->address;

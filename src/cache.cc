...
void CACHE::handle_fill()
{
  ...
  if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu])
  {
    ...
    {
      set = get_set(MSHR.entry[mshr_index].address,NORMAL_HIERARCHY);
      way = timestamp_victim(fill_cpu, MSHR.entry[mshr_index].instr_id, set, block[set], MSHR.entry[mshr_index].ip, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].type);
    }
    ...
    { // this is a bypass that does not fill the LLC
      ...
      // update replacement policy, only for normal hierarchy
      if (cache_nature == NORMAL_HIERARCHY)
      {
        if (cache_type == IS_LLC)
        {
    ...
    else
    {
      if (do_fill)
      {
        ...
        if (cache_type == IS_ITLB)
        {
          MSHR.entry[mshr_index].instruction_pa = spec_block[set][way].data;
          if (PROCESSED.occupancy < PROCESSED.SIZE)
            PROCESSED.add_queue(&MSHR.entry[mshr_index]);
        }
        else if (cache_type == IS_DTLB)
        {
          MSHR.entry[mshr_index].data_pa = spec_block[set][way].data;
          if (PROCESSED.occupancy < PROCESSED.SIZE)
            PROCESSED.add_queue(&MSHR.entry[mshr_index]);
        }
        else if (cache_type == IS_L1I)
...
void CACHE::handle_writeback()
{
  ...
  if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0))
  {
    ...
    { // writeback miss (or RFO miss for L1D)
      ...
      {
        // find victim
        uint32_t set = get_set(WQ.entry[index].address,NORMAL_HIERARCHY), way;
        if (cache_type == IS_LLC)
        {
        ...
        if (do_fill)
        {
          ...
          fill_cache(set, way, &WQ.entry[index],cache_nature);
    ...
    if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0))
    {
      ...
      { // read miss in normal hierarchy
        ...
        { // read hit in speculative hierarchy
          ...
          {
            RQ.entry[index].data_pa = spec_block[set][way].data;
            if (PROCESSED.occupancy < PROCESSED.SIZE)
              PROCESSED.add_queue(&RQ.entry[index]);
          }
          else if (cache_type == IS_STLB)
            RQ.entry[index].data = spec_block[set][way].data;
          else if (cache_type == IS_L1I)
          {
            if (PROCESSED.occupancy < PROCESSED.SIZE)
              PROCESSED.add_queue(&RQ.entry[index]);
          ...
          {
            if (cache_type == IS_L1I)
              l1i_prefetcher_cache_operate(read_cpu, RQ.entry[index].ip, 1, spec_block[set][way].prefetch);
            if (cache_type == IS_L1D)
              l1d_prefetcher_operate(RQ.entry[index].full_addr, RQ.entry[index].ip, 1, RQ.entry[index].type);
            else if (cache_type == IS_L2C)
              l2c_prefetcher_operate(spec_block[set][way].address << LOG2_BLOCK_SIZE, RQ.entry[index].ip, 1, RQ.entry[index].type, 0);
            else if (cache_type == IS_LLC)
            {
              cpu = read_cpu;
              llc_prefetcher_operate(spec_block[set][way].address << LOG2_BLOCK_SIZE, RQ.entry[index].ip, 1, RQ.entry[index].type, 0);
              cpu = 0;
            }
          }
          ...
          if (cache_type == IS_LLC)
          {
            llc_update_replacement_state(read_cpu, set, way, spec_block[set][way].full_addr, RQ.entry[index].ip, 0, RQ.entry[index].type, 1);
          }
          else
            update_replacement_state(read_cpu, set, way, spec_block[set][way].full_addr, RQ.entry[index].ip, 0, RQ.entry[index].type, 1);
          ...
          // update prefetch stats and reset prefetch bit
          if (spec_block[set][way].prefetch)
          {
            pf_useful++;
            spec_block[set][way].prefetch = 0;
          }
          spec_block[set][way].used = 1;
...
void CACHE::handle_prefetch()
{
  ...
  for (uint32_t i = 0; i < MAX_READ; i++)
  {
    ...
    if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0))
    {
      ...
      // access cache
      uint32_t set = get_set(PQ.entry[index].address,NORMAL_HIERARCHY);
      int way = check_hit(&PQ.entry[index],NORMAL_HIERARCHY);
...
uint32_t CACHE::get_set(uint64_t address, uint8_t cache_nature)
{
  if (cache_nature == NORMAL_HIERARCHY)
...
void CACHE::fill_cache(uint32_t set, uint32_t way, PACKET *packet, uint8_t cache_nature)
{
#ifdef SANITY_CHECK
  if (cache_type == IS_ITLB)
  {
  ...
  {
    if (spec_block[set][way].valid == 0)
      spec_block[set][way].valid = 1;
    spec_block[set][way].dirty = 0;
    spec_block[set][way].used = 0;

    spec_block[set][way].delta = packet->delta;
    spec_block[set][way].depth = packet->depth;
    spec_block[set][way].signature = packet->signature;
    spec_block[set][way].confidence = packet->confidence;

    spec_block[set][way].tag = packet->address;
    spec_block[set][way].address = packet->address;
    spec_block[set][way].full_addr = packet->full_addr;
    spec_block[set][way].data = packet->data;
    spec_block[set][way].ip = packet->ip;
    spec_block[set][way].cpu = packet->cpu;
    spec_block[set][way].instr_id = packet->instr_id;

    DP(if (warmup_complete[packet->cpu]) {
      cout << "[" << NAME << "] " << __func__ << " set: " << set << " way: " << way;
      cout << " lru: " << spec_block[set][way].lru << " tag: " << hex << spec_block[set][way].tag << " full_addr: " << spec_block[set][way].full_addr;
      cout << " data: " << spec_block[set][way].data << dec << endl; });
  }
}

int CACHE::check_hit(PACKET *packet, uint8_t cache_nature)
{
  uint32_t set = get_set(packet->address,cache_nature);
  int match_way = -1;
  ...
  else
  {
    ...
    for (uint32_t way = 0; way < NUM_WAY_SPEC; way++)
    {
      ...
      if (blk.valid && (blk.tag == packet->address) && (blk.timestamp <= packet->timestamp))
      {
...
int CACHE::invalidate_entry(uint64_t inval_addr, uint8_t cache_nature)
{

  int match_way = -1;
  uint32_t set = get_set(inval_addr,cache_nature);
  ...
  else
  {
    ...
    for (uint32_t way = 0; way < NUM_WAY_SPEC; way++)
    {
      if (spec_block[set][way].valid && (spec_block[set][way].tag == inval_addr))
      {
        ...
        DP(if (warmup_complete[cpu]) {
              cout << "[" << NAME << "] " << __func__ << " inval_addr: " << hex << inval_addr;  
              cout << " tag: " << spec_block[set][way].tag << " data: " << spec_block[set][way].data << dec;
              cout << " set: " << set << " way: " << way << " lru: " << spec_block[set][way].lru << " cycle: " << current_core_cycle[cpu] << endl; });

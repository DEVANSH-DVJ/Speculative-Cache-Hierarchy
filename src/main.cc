...
uint64_t va_to_pa(uint32_t cpu, uint64_t instr_id, uint64_t va, uint64_t unique_vpage, uint8_t is_code)
{
    ...
    { // no VA => PA translation found
        ...
        { // not enough memory
            ...
            ooo_cpu[cpu].ITLB.invalidate_entry(NRU_vpage,NORMAL_HIERARCHY);
            ooo_cpu[cpu].DTLB.invalidate_entry(NRU_vpage,NORMAL_HIERARCHY);
            ooo_cpu[cpu].STLB.invalidate_entry(NRU_vpage,NORMAL_HIERARCHY);
            for (uint32_t i = 0; i < BLOCK_SIZE; i++)
            {
                uint64_t cl_addr = (mapped_ppage << 6) | i;
                ooo_cpu[cpu].L1I.invalidate_entry(cl_addr,NORMAL_HIERARCHY);
                ooo_cpu[cpu].L1D.invalidate_entry(cl_addr,NORMAL_HIERARCHY);
                ooo_cpu[cpu].L2C.invalidate_entry(cl_addr,NORMAL_HIERARCHY);
                uncore.LLC.invalidate_entry(cl_addr,NORMAL_HIERARCHY);
            }

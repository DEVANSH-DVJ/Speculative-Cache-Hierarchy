        ...
        for (uint32_t i = head; i < tail; i++)
        {
            ...
            else
            {
                if (entry[i].address == packet->address)
                {
                    ...
                    // cout << "[" << NAME << "] " << __func__ << " cpu: " << packet->cpu << " instr_id: " << packet->instr_id << " same address: " << hex << packet->address;
                    // cout << " full_addr: " << packet->full_addr << " physical_addr: "<< packet->full_physical_address << " virtual_addr: " << packet->full_virtual_address << dec << " by instr_id: " << entry[i].instr_id << " index: " << i;
                    // cout << " cycle " << packet->event_cycle << endl; }
                    return i;
                }
            }

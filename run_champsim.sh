...
mkdir -p $(dirname results_${N_SIM}M/${BINARY}/${TRACE}-${BINARY}${OPTION}.txt)
(./bin/${BINARY} -warmup_instructions ${N_WARM}000000 -simulation_instructions ${N_SIM}000000 ${OPTION} -traces ${TRACE_DIR}/${TRACE}) &> results_${N_SIM}M/${BINARY}/${TRACE}-${BINARY}${OPTION}.txt
